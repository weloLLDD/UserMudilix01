...
export const listProduct = (keyword="",pageNumber="") => async(dispatch) =>{
    try {
        ...
        const {data} = await axios.get(`https://backmudilix-12.onrender.com/api/products?keyword=${keyword}&pageNumber=${pageNumber}`);
...
export const listProductDetails = (id) => async(dispatch) =>{
    try {
        ...
        const {data} = await axios.get(`https://backmudilix-12.onrender.com/api/products/${id}`);
 ...
 //update PRODUCT
   export const editProduct = (product) => async (dispatch, getState) => {
    try {
      ...
      const {data} = await axios.put(`https:/backmudilix-12.onrender.com/api/products/${product._id}`,product, config);
